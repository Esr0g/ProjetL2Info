...
Uint32 bougerEnnemisDebut1(Uint32 intervalle, void *parametre) {
	...
	Ennemi *e = NULL;
		...
		int y_etape =256;
		int j =0;
		int** chemain_1 =PassFinding_chemain_1("niveau/niveau_1");
	...
	for (int i = 0; i < listeTailleEn(*li); i++) {
		...
		//creation de l'objectif de deplacement par retracement du chemain
		for(j=0; j<= e->moment_chemin+1;j++){//retrouver le x y qu'il doit atteidre 
			// ATTENTION le x et y  du mob est inverser par rapport au x et y de la "map".txt
			// doit venir de comment est coder soit le "carrerMob" ou son déplacement
			if(j < 30){
				if(chemain_1[j][0] ==1 ){
					y_etape +=chemain_1[j][1] *66;
				}
				else if(chemain_1[j][0] ==2 ){
					x_etape += chemain_1[j][1] *66;				
				}
				else if(chemain_1[j][0] ==3 ){
					x_etape -= chemain_1[j][1] *66;
				}
				else if(chemain_1[j][0] ==4){
					y_etape -= chemain_1[j][1] *66;				
				}	
			}
		...
		// deplacement vers la fin du chemain qui se fait emprinté
		// ou dit qu'on est arrivé a un autre chemain
		if(chemain_1[e->moment_chemin][0] ==1 ){
			if(e->forme.y < y_etape ){
				e->forme.y++;
				//printf("dplc vers BAS\n");
				//printf("x_etape:%d\n",x_etape );
			}else{e->moment_chemin++; }// si on ne peux plus marcher c'est qu'on passe a l'étape suivantes
		}
		else if(chemain_1[e->moment_chemin][0] ==2 ){
			if(e->forme.x  < x_etape){
				e->forme.x++;
				//printf("dplc vers DROITE\n");
				//printf("y_etape:%d\n",y_etape );
			}else{e->moment_chemin++; }				
		}
		else if(chemain_1[j][0] ==3){
			if (e->forme.x >= x_etape){
				e->forme.x--;
				//printf("dplc vers GAUCHE\n");
			}else{e->moment_chemin++; }
			...
			if(e->forme.y  >= y_etape){
				e->forme.y--;
				//printf("dplc vers HAUT\n");
			}else{e->moment_chemin++; }
		}
		if(e->forme.x >1300 || e->forme.y >1300){
			supprimerEnnemi(li, i);
		}
		//printf("moment_chemin: %d \n",e->moment_chemin );
	}
	free(chemain_1);
...
Uint32 bougerEnnemisDebut2(Uint32 intervalle, void *parametre) {
	...
	Ennemi *e = NULL;
		...
		int y_etape =0;
		int j =0;
		int** chemain_2 =PassFinding_chemain_2("niveau/niveau_1");
	...
	for (int i = 0; i < listeTailleEn(*li); i++) {
		...
		for(j=0; j<= e->moment_chemin+1;j++){//retrouver le x y qu'il doit atteidre 
			...
			if(j < 30){
				if(chemain_2[j][0] ==1 ){
					y_etape +=chemain_2[j][1] *66;
				}
				else if(chemain_2[j][0] ==2 ){
					x_etape += chemain_2[j][1] *66;				
				}
				else if(chemain_2[j][0] ==3 ){
					x_etape -= chemain_2[j][1] *66;
				}
				else if(chemain_2[j][0] ==4){
					y_etape -= chemain_2[j][1] *66;				
				}	
			}
		...
		// deplacement vers la fin du chemain qui se fait emprinté
		// ou dit qu'on est arrivé a un autre chemain
		if(chemain_2[e->moment_chemin][0] ==1 ){
			if(e->forme.y < y_etape ){
				e->forme.y++;
				//printf("dplc vers BAS\n");
				//printf("x_etape:%d\n",x_etape );
		...
		else if(chemain_2[e->moment_chemin][0] ==2 ){
			if(e->forme.x  < x_etape){
				e->forme.x++;
				//printf("dplc vers DROITE\n");
				//printf("y_etape:%d\n",y_etape );
